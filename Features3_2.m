...
%     if Dataset==1
%         load(['Decoding_Accuracy_DS_Mine_Subject_',num2str(Subject),'.mat'],'accuracy');
%     elseif Dataset==2
%         load(['Decoding_Accuracy_DS_Vahab_Subject_',num2str(Subject),'.mat'],'accuracy');
%     elseif Dataset==3
%         load(['Decoding_Accuracy_DS_Stanford_Subject_',num2str(Subject),'.mat'],'accuracy');
%     end
%     for feature=[1:9 11:35]
    for feature=[31:35]
        if feature==33
            ...
            for category = 1:size(signal,3)
                for trial = 1:size(signal,4)
                    ...
                    elseif feature==34 && channel==1 && category==1 && trial==1
                        feature_chosen=nan.*ones(size(signal,1),size(signal,3),size(signal,4),1000);
                    elseif feature==35 && Dataset<3 && channel==1 && category==1 && trial==1
...
%                             Kmax = floor(maxtime./2);
        ...
        clearvars -except Dataset Fs True_Predicted_labels accuracy Subject signal feature X Y  net
...
%                 True_Predicted_labels{feature,combination,fld,2}=PredictedLabels;
            ...
            [Subject feature combination]
        end
        if Dataset==1
            save(['Decoding_Accuracy_DS_Mine_Subject_',num2str(Subject),'.mat'],'accuracy');
        elseif Dataset==2
            save(['Decoding_Accuracy_DS_Vahab_Subject_',num2str(Subject),'.mat'],'accuracy');
        elseif Dataset==3
            save(['Decoding_Accuracy_DS_Stanford_Subject_',num2str(Subject),'.mat'],'accuracy');
        end
    end
end
