...
accuracies_cued=nan*ones(21,231,4,10);
cued=1;
for Subject=[1:10]
%     load(['Corrected_Dec_DS_',Datasets{Dataset},'_Cued_',num2str(cued),'_Band_',Bands{band},'_Wind_slid_Subject_',num2str(Subject),'.mat'],'accuracy');
    load(['Corrected_Dec_DS_',Datasets{Time},'_Cued_',num2str(cued),'_Band_',Bands{band},'_Wind_slid_Subject_',num2str(Subject),'_5ms.mat'],'accuracy');
    accuracy(accuracy==0)=nan;
    accuracies_cued(:,:,1,Subject)=squeeze(nanmean(nanmean(accuracy(features,:,[1 2 3],:),4),3));
    accuracies_cued(:,:,2,Subject)=squeeze(nanmean(nanmean(accuracy(features,:,[1 4 5],:),4),3));
    accuracies_cued(:,:,3,Subject)=squeeze(nanmean(nanmean(accuracy(features,:,[2 4 6],:),4),3));
    accuracies_cued(:,:,4,Subject)=squeeze(nanmean(nanmean(accuracy(features,:,[3 5 6],:),4),3));
    [~,Behavioural_RT_cued(:,Subject)]= DatasetLoading_DS2_behaviour(Subject,cued); % category, percentage
end
% Correlation across subjects
Decoding_RT_correlaation_all=nan*ones(231,21);
for time=1:231
    for feat=[1:21]
...
%     end
% end
% save('random_RT_Behaav_correlations_5ms.mat','Decoding_RT_correlaation_all_random')
load('random_RT_Behaav_correlations_5ms.mat','Decoding_RT_correlaation_all_random');
threshold=0.9;
for feat=[1:21]
    for time=1:231      
...
save('Mean_decoding_behav_correlation_each_feature.mat','data');
ccc
...
%     end
% end
% save('random_RT_Behaav_correlations_feat_combin_5ms.mat','Decoding_RT_correlaation_all_random')


load('random_RT_Behaav_correlations_feat_combin_5ms.mat','Decoding_RT_correlaation_all_random');
threshold=0.9;
for feat=1:17
    for time=1:231      
        if Decoding_RT_correlaation_all(time,feat)>=0 && sum(Decoding_RT_correlaation_all(time,feat)>Decoding_RT_correlaation_all_random(time,feat,:))>threshold*size(Decoding_RT_correlaation_all_random,3)
