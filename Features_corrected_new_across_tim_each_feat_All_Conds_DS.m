...
for band=bands   % 1=broad, 2=delta, 3=theta, 4=alpha, 5=beta, 6=gamma
    ...
    for cued=0:1
        for Subject=Subjects
            ...
            accuracy = nan*ones(1,length(Windows),nchoosek(size(signal,3),2),10);
            for windoww=Windows
                ...
                for feature=[2:9 11:13 18:27] % features when evaluating time windows
                    ...
                    clearvars -except Windows cued Subjects bands lowband highband band windows windoww winds Dataset Fs True_Predicted_labels accuracy Subject signal feature X Y  net
                    ...
                    for combination=1:size(combinations,1)
                        ...
                        for fld=1:folds
                            ...
                            Classifier_Model = fitcdiscr(Xtrain,indx,'DiscrimType','pseudolinear');
                            ...
                            accuracy(feature,windoww,combination,fld)=sum(PredictedLabels==Ytest)./length(Ytest);
                            %                 True_Predicted_labels{feature,combination,fld,1}=Ytest;
                            %                 True_Predicted_labels{feature,combination,fld,2}=PredictedLabels;
                        end
                        [cued Subject windoww feature combination]
                    end                    
                    Datasets={'Mine','Vhab','Stfd'};
                    Bands={'Broad','Delta','Theta','Alpha','Betta','Gamma'};
                end
                save(['Corrected_Dec_DS_',Datasets{Dataset},'_Cued_',num2str(cued),'_Band_',Bands{band},'_Wind_slid_Subject_',num2str(Subject),'5ms.mat'],'accuracy');
            end
        end
    end
end
