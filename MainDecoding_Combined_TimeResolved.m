...
clear all;
% close all;
Dataset=1; %1:3: which dataset to use

Subjects=[1:10];% There are 10 subjects per dataset
bands=[1]; % Apply any band-pass filtering? 1=broad, 2=delta, 3=theta, 4=alpha, 5=beta, 6=gamma
Windows=[1:231]; % Number of decoding analyses performed in the trial considering the step size
...
for band=bands   % 1=broad, 2=delta, 3=theta, 4=alpha, 5=beta, 6=gamma
    ...
    for Feat_selection=[1:9 12:19]
        for Subject=Subjects
            ...
            Sel_feat=zeros(231,15,26); % feature-extracted dataset size: 231 time points* 15 combination of categories (for dataset 3) * 26 features
            accuracy = nan*ones(1,nchoosek(size(signal,3),2),length(Windows));
            for windoww=Windows
                
                windows=[1:1200]; % time window of analysis + 200 ms
                step_size=5; % analysis step size
                window_span=50; % length of sliding time window
                wind=windows((windoww-1)*step_size+1:(windoww-1)*step_size+window_span);
                
                features_chosen=nan*ones(size(signal,1),size(signal,3),size(signal,4),26);
                feat=0;
                for feature=[2:9 11:13 18:30 32 34] % features
                    feat=feat+1;
                        ...
                        for category = 1:size(signal,3)
                            for trial = 1:size(signal,4)
                                ...
                                if sum(isnan(trial_data))>0
                                    ...
                                    elseif feature==10
                                        %%                  Lyapunov exponent (largest LLE): not used
                                    ...
                                    elseif feature==31
                                        %% Phase-Amplitude Coupling: not used
                    ...
                    if feature>27 %  Reduce the size of the multi-valued features to one using PCA: first PCA round
                        features_chosen_tmp=[];
                        for Feat_sel=1:size(feature_chosen,4)
                            features_chosen_tmp=vertcat(features_chosen_tmp,reshape(feature_chosen(:,:,:,Feat_sel),[size(signal,1) size(signal,3)*size(signal,4)]));
                        end
                        nans=isnan(features_chosen_tmp(1,:));
                        label_tmp=repmat(1:size(signal,3),[1 size(signal,4)]);
                        label_tmp(nans)=[];
                        rmpath(genpath('/Analyses/FSLib_v6.2.1_2018'))
                        features_reshaped_tmp=features_chosen_tmp(1:sum(~isnan(features_chosen_tmp(:,1))),logical(1-nans))';
                        coeff =pca(features_reshaped_tmp);
                        features_reshaped_tmp = features_reshaped_tmp*coeff(1:size(signal,1),:);
                        
                        feature_chosen_tmp2=nan*ones(size(signal,3)*size(signal,4),size(signal,1));
                        g=0;
                        for i=1:size(signal,3)*size(signal,4)
                            if nans(i)==0
                                g=g+1;
                                feature_chosen_tmp2(i,:)=features_reshaped_tmp(g,:);
                            end
                        end
                        for chch=1:size(signal,1)
                            feature_chosenT(chch,:,:)=reshape(feature_chosen_tmp2(:,chch),[size(signal,3) size(signal,4)]);
                        end
                        feature_chosen=feature_chosenT;
                        addpath(genpath('/FSLib_v6.2.1_2018'))
                    end
                    
                    features_chosen(:,:,:,feat)=zscore(feature_chosen);
                end
                
                %% Reshpaing the features into a data matrix X
                features_chosen_tmp=[];
                ...
                %% Classification
                clearvars -except Feat_selection Sel_feat Windows Subjects bands lowband highband band windows windoww wind Dataset Fs True_Predicted_labels accuracy Subject signal feature X Y  net
                ...
                for combination=1:size(combinations,1)
                    ...
                    rmpath(genpath('/Analyses/FSLib_v6.2.1_2018')) % remove the FS package from matlab paths to avoid interference with Matlab PCA function
                    t=zeros(size(XX,1),26);
                    % applying PCA to reduce the dimension of each feature
                    % to 1 for Feature Selection: second PCA round
                    for feats=1:26
                        dd=XX(:,(feats-1)*size(signal,1)+1:feats*size(signal,1));
                        coeff =pca(dd);
                        t(:,feats) = dd*coeff(1,:);
                    end
                    
                    % selecting the best features using different FS
                    % algorithms
                    addpath(genpath('/Analyses/FSLib_v6.2.1_2018')) % remove the FS package from matlab paths to avoid interference with Matlab PCA function
                    [fs,selection_method]=Feature_Selection_Ultimate(t,YY,Feat_selection,5);
                    rmpath(genpath('/Analyses/FSLib_v6.2.1_2018')) % adding back the FS package to matlab paths
                    
                    % concatanating the selected features in a data matrix
                    XXX=[];
                    for feats=fs
                    ...
                    % concatanating selected features and reducing the dimenion of the final data matrix to equal the number of channels in the dataset: third PCA round
                    coeff =pca(XXX);
                    XXX = XXX*coeff(1:size(signal,1),:);
                    
                    Xready=XXX;
                    Yready=YYY;
                    Sel_feat(windoww,combination,fs)=1; % recording the selected features for presentations
                    ...
                    [band Feat_selection Subject windoww combination]
                end
                Datasets={'Dataset1','Dataset2','Dataset3'};
                Bands={'Broad','Delta','Theta','Alpha','Betta','Gamma'};
            end
            save(['Corrected_Dec_DS_',Datasets{Dataset},'_Band_',Bands{band},'_Wind_sliding_Subject_',num2str(Subject),'_CombFeat_',selection_method,'_PCA_cmplt.mat'],'accuracy','Sel_feat');
        end
    end
end
