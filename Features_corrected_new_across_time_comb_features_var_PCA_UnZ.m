...
%             X=features_chosen_tmp(Features_selected,:)';
            ...
            for combination=1:size(combinations,1)
                ...
                %                 opts = statset('Display','iter');
                %                 fun = @(XT,yT,Xt,yt)loss(fitcecoc(XT,yT),Xt,yt);
                %                 [fs,history] = sequentialfs(fun,XX,YY','cv',c,'options',opts);
                %                 XXX=XX(:,fs);
                %                 YYY=YY;
                %                 Xready=XXX;
                %                 Yready=YYY;
...
%                 clearvars YY XX
        ...
        save(['Corrected_Dec_DS_',Datasets{Dataset},'_Band_',Bands{band},'_Wind_sliding_Subject_',num2str(Subject),'_CombFeat_Varnc_PCA_UnZ.mat'],'accuracy','Sel_feat');
    end
end
