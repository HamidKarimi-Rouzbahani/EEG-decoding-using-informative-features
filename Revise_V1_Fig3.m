...
for Feature=chosen_features
    p=p+1;
        plot_line(p)=plot(times,smooth(nanmean(accuracies(Feature,:,:),3),5),'Color',colors{p},'linewidth',2);
    hold on;
end
...
for feature=chosen_features
    ...
    for windoww=1:size(Bayes,2)
        if Bayes(feature,windoww)==-0.5 || Bayes(feature,windoww)==0.5
            plots(f)=plot(times(windoww),Bayes(feature,windoww).*steps+Baseline-(f-1)*(3*2+distans)*steps,'LineStyle','none','marker','o','Color',colors{f},'linewidth',2,'markersize',2);
        elseif Bayes(feature,windoww)~=0
            plots(f)=plot(times(windoww),Bayes(feature,windoww).*steps+Baseline-(f-1)*(3*2+distans)*steps,'LineStyle','none','marker','o','MarkerFaceColor',colors{f},'Color',colors{f},'linewidth',2,'markersize',2);
        end
    end
...
elseif series==2
    legend([plot_line(1),plot_line(2),plot_line(3),plot_line(4),plot_line(5),plot_line(6),plot_line(7),plot_line(8),plot_line(9)],...
        {Feat_names{array(1)},Feat_names{array(2)},Feat_names{array(3)},Feat_names{array(4)},Feat_names{array(5)},Feat_names{array(6)},Feat_names{array(7)},Feat_names{array(8)},Feat_names{array(9)}},'EdgeColor','w','FontSize',12);
