...
for Subject=1:10
    ...
    for window=1:7
        wind=windows(window,:);
        for feature=[2:9 11:13 18:30 32 34:35]
%         for feature=[35]
            if feature==33
                ...
                for category = 1:size(signal,3)
                    for trial = 1:size(signal,4)
                        ...
                        elseif (feature==29 || feature==30) && channel==1 && category==1 && trial==1
                            feature_chosen=nan.*ones(size(signal,1),size(signal,3),size(signal,4),100);
            ...
            clearvars -except windows window wind Dataset Fs True_Predicted_labels accuracy Subject signal feature X Y  net
            ...
            for combination=1:size(combinations,1)
                ...
                [Subject window feature combination]
            end
            if Dataset==1
                save(['Decoding_Accuracy_DS_Mine_Window_',num2str(window),'_Subject_',num2str(Subject),'.mat'],'accuracy');
            elseif Dataset==2
                save(['Decoding_Accuracy_DS_Vahab_Window_',num2str(window),'_Subject_',num2str(Subject),'.mat'],'accuracy');
            elseif Dataset==3
                save(['Decoding_Accuracy_DS_Stanford_Window_',num2str(window),'_Subject_',num2str(Subject),'.mat'],'accuracy');
            end
        end
    end
end
