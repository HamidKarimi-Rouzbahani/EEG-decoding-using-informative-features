...
function [Subj]=Features_cambridge_across_time_Theta_band_only_flow2(Subj)
% Subj=1
Dataset=3;

Windows=[1:231];
Fs=1000;
band=[3]; % 1=broad, 2=delta, 3=theta, 4=alpha, 5=beta, 6=gamma
lowband=4;
highband=8;
Subject=Subj;
...
deltas = nan*ones(35,nchoosek(size(signal,3),2),length(Windows),128);
for windoww=Windows
    %     tic
    windows=[1:1200];
    step_size=5;
    ...
    for feature=[2] % features when evaluating time windows
        ...
        clearvars -except deltas Windows Subj Subjects bands lowband highband band windows windoww wind Dataset Fs True_Predicted_labels accuracy Subject signal feature X Y  net
...
save(['Revise_corrected_Dec_DS_',Datasets{Dataset},'_Band_Theta_Wind_sliding_Subject_',num2str(Subject),'_Wave_Camb.mat'],'accuracy','deltas');
end
