...
% Subjects=[1:10];
%  for Subject=1
function [Subject]=Revised_features_corrected_new_across_time_bands(Subject)
Dataset=1;
...
for band=bands   % 1=broad, 2=delta, 3=theta, 4=alpha, 5=beta, 6=gamma
    ...
    signal = DatasetLoading_cambridge(Dataset,Subject); % channel, time, cat, trial
    ...
    for windoww=Windows
        ...
        for feature=[36:37] % features when evaluating freq bands
                ...
                for category = 1:size(signal,3)
                    for trial = 1:size(signal,4)
                        ...
                        elseif feature==35 && Dataset<3 && channel==1 && category==1 && trial==1 % Autocorr
                            feature_chosen=nan.*ones(size(signal,1),size(signal,3),size(signal,4),size(signal,1));
                        elseif feature==36 && channel==1 && category==1 && trial==1 % ICC
                            feature_chosen=nan.*ones(size(signal,1),size(signal,3),size(signal,4),size(signal,1));
                        elseif feature==37 && channel==1 && category==1 && trial==1 % ICC
                            feature_chosen=nan.*ones(size(signal,1),size(signal,3),size(signal,4),size(signal,1));
                        end
                        ...
                        if sum(isnan(trial_data))>0
                            ...
                            elseif feature==36
                                ...
                                Hilb1 = abs(hilbert(trial_data));
                                AAC=nan(size(signal,1),1);
                                for ch2=search_light
                                    trial_data2=signal(ch2,wind,category,trial);
                                    if band>1
                                        trial_data2=eegfilt(trial_data2,Fs,lowband,highband,0,floor(length(trial_data2)/3),0,'fir1');
                                    end
                                    Hilb2 = abs(hilbert(trial_data2));
                                    AAC(ch2,1)=corr(Hilb1',Hilb2');
                                end
                                feature_chosen(channel,category,trial,:)=AAC;
                            elseif feature==37
                                %% Phase-Phase Coupling
                                Hilb1 = angle(hilbert(trial_data));
                                PPC=nan(size(signal,1),1);
                                ...
                                for ch2=search_light
                                    ...
                                    Hilb2 = angle(hilbert(trial_data2));
                                    delta_phase=Hilb1-Hilb2;
                                    PPC(ch2,1)=abs(mean(exp(1j*delta_phase)));
                                end
                                feature_chosen(channel,category,trial,:)=PPC;                                end
                        end
                    end
                end
            end
            ...
            for cat=1:size(signal,3)
                ...
                gg=[];
                counter_trial=1:size(feature_chosen,3);
                for trial=counter_trial(isnotnan(1,1,:,2))
                    tt=[];
                    for ch=1:size(feature_chosen,1)
                        counter_feat=1:size(feature_chosen,1);
                        for feat=counter_feat(isnotnan(ch,1,1,:))
                            tt=horzcat(tt,feature_chosen(ch,cat,trial,feat));
                        end
                    end
                    gg=vertcat(gg,tt);
                end
                Xt=vertcat(Xt,gg);
                Y=vertcat(Y,cat*ones(size(gg,1),1));
            end
            X=Xt;
