...
for band=1:size(Bayes,2)
    for feature=1:size(Bayes,1)
        ...
        if Bayes(feature,band)==-0.5 || Bayes(feature,band)==0.5
            plots(band)=plot(xtic(feature)+band/4,Bayes(feature,band).*steps+Baseline-(3*2+distans)*steps,'LineStyle','none','marker','o','Color',colors{band},'linewidth',2,'markersize',4);
        elseif Bayes(feature,band)~=0
            plots(band)=plot(xtic(feature)+band/4,Bayes(feature,band).*steps+Baseline-(3*2+distans)*steps,'LineStyle','none','marker','o','MarkerFaceColor',colors{band},'Color',colors{band},'linewidth',2,'markersize',4);
        end
    end
