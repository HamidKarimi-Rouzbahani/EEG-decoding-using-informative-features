...
set(gca,'FontSize',12,'FontName','Calibri','XTick',...
...
%% Bar plots
gca = axes('Position',[0.145 0.45 0.5 0.55]);%x position, y position, width and height
...
set(gca,'FontSize',16,'FontName','Calibri','XTick',xtic+2,'XTickLabel',{''},...
...
set(gca,'FontSize',13,'FontName','Calibri','XTick',...
    [xtic]+2,'XTickLabel',Feat_names,'YTick',...
...
for Dataset=1:3
    ...
    for feat1=[1 23 28]
        ...
        for feat2=[1 23 28]
            f2=f2+1;
            Significanc_mat(f1,f2)=bf.ttest(squeeze(data(feat1,Dataset,:)),squeeze(data(feat2,Dataset,:)));
            %         if feat1==feat2
            %             Significanc_mat(feat1,feat2)=1;
            %         end
            if f1==f2
                Significanc_mat(f1,f2)=0.01;
            end
            if f1<f2
                Significanc_mat(f1,f2)=0;
            end
    ...
    for f1=[1:3]
        for f2=[1:3]
            if Significanc_mat(f1,f2)>10
                Bayes_mat(f1,f2,Dataset)=6;
            elseif Significanc_mat(f1,f2)>3 && Significanc_mat(f1,f2)<=10
                Bayes_mat(f1,f2,Dataset)=5;
            elseif Significanc_mat(f1,f2)>1 && Significanc_mat(f1,f2)<=3
                Bayes_mat(f1,f2,Dataset)=4;
            elseif Significanc_mat(f1,f2)<1 && Significanc_mat(f1,f2)>=1/3
                Bayes_mat(f1,f2,Dataset)=3;
            elseif Significanc_mat(f1,f2)<1/3 && Significanc_mat(f1,f2)>=1/10
                Bayes_mat(f1,f2,Dataset)=2;
            elseif Significanc_mat(f1,f2)<1/10 && Significanc_mat(f1,f2)~=0
                Bayes_mat(f1,f2,Dataset)=1;
            elseif Significanc_mat(f1,f2)==0
                Bayes_mat(f1,f2,Dataset)=0;
            end
        end
    end
    subplot_tmp=subplot(3,1,Dataset);
    hold(subplot_tmp,'on');
