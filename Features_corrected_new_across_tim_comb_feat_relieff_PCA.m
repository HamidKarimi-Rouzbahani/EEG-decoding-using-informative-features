...
%                 features_chosen(:,:,:,feat)=(feature_chosen);
            end
...
%             mdl = fitglm(features_chosen_tmp(1:size(signal,1),:)',Y);
%             Ref_adj_r_squared=mdl.Rsquared.Adjusted;
%          
%             X=features_chosen_tmp(1:size(signal,1),:);
%             for Feat_sel=2:size(features_chosen,4)
%                 feature_tmp=reshape(features_chosen(:,:,:,Feat_sel),[size(signal,1) size(signal,3)*size(signal,4)]);
%                 feature_tmp(:,nans)=[];
%                 mdl = fitglm([features_chosen_tmp(1:size(signal,1),:);feature_tmp]',Y);
%                 Feat_adj_r_squared=mdl.Rsquared.Adjusted;
%                 
%                 if Feat_adj_r_squared>Ref_adj_r_squared
%                     X=vertcat(X,feature_tmp);
%                     Sel_feat(windoww,Feat_sel)=1;
%                 end
%             end
%             X=X';
%             X=features_chosen_tmp';
...
%             t = score*coeff(1:size(signal,1),:)' + repmat(mu(1:size(signal,1)),size(X,1),1);
%             X=t;
            %             for i=1:size(features_chosen_tmp,1)
            %                 cors(i)=abs(corr(Y',features_chosen_tmp(i,:)','Type','Spearman','Rows','complete'));
%             end
%             [feat,numb]=sort(cors,'descend');
%             Features_selected=fs;                        
%             X=features_chosen_tmp(Features_selected,:)';

            %% Classification
            clearvars -except Sel_feat Windows Subjects bands lowband highband band windows windoww wind Dataset Fs True_Predicted_labels accuracy Subject signal feature X Y  net
            ...
            for combination=1:size(combinations,1)
                ...
                %% regression based all channels added simultaeously
%                 mdl = fitglm(XX(:,1:size(signal,1)),YY);
%                 Ref_adj_r_squared=mdl.Rsquared.Adjusted;
%                 
%                 XXX=XX(:,1:size(signal,1));
%                 for Feat_sel=2:21
%                     feature_tmp=XX(:,(Feat_sel-1)*size(signal,1)+1:Feat_sel*size(signal,1));
%                     mdl = fitglm(horzcat(XX(:,1:size(signal,1)),feature_tmp),YY);
%                     Feat_adj_r_squared=mdl.Rsquared.Adjusted;
%                     
%                     if Feat_adj_r_squared>Ref_adj_r_squared
%                         XXX=horzcat(XXX,feature_tmp);
%                         Sel_feat(windoww,combination,Feat_sel)=1;
%                     end
%                 end
%                 Xready=XXX;
%                 Yready=YY;            
%                 [coeff,score,latent,tsquared,explained,mu] =pca(Xready);
%                 t = score*coeff(1:size(signal,1),:)' + repmat(mu(1:size(signal,1)),size(Xready,1),1);
%                 Xready=t;
                %%                 Serial adding of features each channel is evaluated
                
                t=zeros(size(XX,1),21);
                for feats=1:21
                    [coeff,score,latent,tsquared,explained,mu] =pca(XX(:,(feats-1)*size(signal,1)+1:feats*size(signal,1)));
                    t(:,feats) = score*coeff(1,:)' + repmat(mu(1),size(XX,1),1);
                end
                
%                 % Complete
%                 cvp = cvpartition(YY,'holdout',ceil(length(YY)./10));
%                 Xtrain = t(cvp.training,:);
%                 ytrain = YY(cvp.training);
%                 Xtest  = t(cvp.test,:);
%                 ytest  = YY(cvp.test);            
...
%                 tol    = 0.02;
%                 fs = find(nca.FeatureWeights > tol*max(1,max(nca.FeatureWeights))); 
                
%                 % simplified
%                 mdl = fscnca(t,YY,'Solver','sgd','Verbose',1);
%                 [~,feats]=sort(mdl.FeatureWeights,'descend');


                % relieff
                [idx,scores] = relieff(t,YY',10);
                fs=idx(1:5);
                XXX=[];
                for feats=fs
                ...
                [band Subject windoww combination]
            end
            Datasets={'Mine','Vhab','Stfd'};
            Bands={'Broad','Delta','Theta','Alpha','Betta','Gamma'};
        end
        save(['Corrected_Dec_DS_',Datasets{Dataset},'_Band_',Bands{band},'_Wind_sliding_Subject_',num2str(Subject),'_CombFeat_relieff_PCA.mat'],'accuracy','Sel_feat');
    end
end
...
for windoww=Windows
    for Subject=1:10
        load(['Corrected_Dec_DS_',Datasets{Dataset},'_Band_',Bands{band},'_Wind_sliding_Subject_',num2str(Subject),'_SelFeat.mat'],'accuracy','Sel_feat');
