...
%                 Xready=t;
                ...
                c = cvpartition(YY','k',10);
                opts = statset('Display','iter','UseParallel',true);
                fun = @(XT,yT,Xt,yt)loss(fitcecoc(XT,yT),Xt,yt);
                [fs,history] = sequentialfs(fun,t,YY','cv',c,'options',opts);
                fs(1,1)=1;
                featss=[1:21];
                featss=featss(fs);
                XXX=[];
                ...
                Xready=XXX;
                Yready=YYY;
                Sel_feat(windoww,combination,:)=fs;
...
%                 clearvars YY XX
        ...
        save(['Corrected_Dec_DS_',Datasets{Dataset},'_Band_',Bands{band},'_Wind_sliding_Subject_',num2str(Subject),'_CombFeat_seq_PCA_MeanIs.mat'],'accuracy','Sel_feat');
    end
end
