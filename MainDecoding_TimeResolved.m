        ...
        for windoww=Windows
            ...
            for feature=[2:9 11:13 18:30 32 34:37] % features
                    ...
                    for category = 1:size(signal,3)
                        for trial = 1:size(signal,4)
                            ...
                            elseif feature==35 && Dataset<3 && channel==1 && category==1 && trial==1 % Autocorr
                                feature_chosen=nan.*ones(size(signal,1),size(signal,3),size(signal,4),size(signal,1));
                            elseif feature==36 && channel==1 && category==1 && trial==1 % Amplitude-Amplitude Coupling
                                feature_chosen=nan.*ones(size(signal,1),size(signal,3),size(signal,4),size(signal,1));
                            elseif feature==37 && channel==1 && category==1 && trial==1 % Phase-Phase Coupling
                                feature_chosen=nan.*ones(size(signal,1),size(signal,3),size(signal,4),size(signal,1));
                            end
                ...
                for combination=1:size(combinations,1)
                    ...
                    deltas(feature,combination,windoww,:)=Classifier_Model.DeltaPredictor;
