...
%                 Xready=t;
                ...
                fun = @(XT,yT,Xt,yt)loss(fitcecoc(XT,yT),Xt,yt);
                [fs,history] = sequentialfs(fun,t,YY','cv',c,'keepin',[1],'options',opts);
                featss=[1:21];
                featss=featss(fs);
                XXX=[];
                for feats=featss
...
%                 clearvars YY XX
        ...
        save(['Corrected_Dec_DS_',Datasets{Dataset},'_Band_',Bands{band},'_Wind_sliding_Subject_',num2str(Subject),'_CombFeat_seq_PCA_MeanIs2.mat'],'accuracy','Sel_feat');
    end
end
